...
const ImageSelector = ({ image, setImage, handleDeleteImg}) => {
    ...
    useEffect(() => {
      // If the image prop is a string (URL), set is as a preview URL
  ...
  return (
    <div>
        ...
        {!image ? <button className='w-full h-[220px] flex flex-col items-center justify-center gap-4 bg-slate-50 rounded border border-slate-200/50' onClick={() => onChooseFile()}>
            <div className='w-14 h-14 flex items-center justify-center bg-primary/15 rounded-full border border-primary-20'>
